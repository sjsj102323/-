...
import gym
import gol
from multi_energy import multi_energyEnv
gol.init()
def built_parser(method):
    ...
    parser.add_argument("--env", default="Muti_energy_env")
    parser.add_argument("--env_name", default="Muti_energy_env")
...
def actor_agent(args, shared_queue, shared_value,share_net, lock, i):
    ...
    actor.run()
    shared_queue[0]=actor.get_experience_in_queue()
...
def main(method):
    ...
    for i in range(args.num_buffers):
        experience_in_queue.append(Queue(maxsize=1000))
    ...
    if args.code_model=="train":
        for i in range(args.num_actors):
            actor_agent(args, shared_queue, shared_value, share_net, lock, i)
        for i in range(args.num_learners):
            #device = torch.device("cuda")
            device = torch.device("cpu")
            leaner_agent(args, shared_queue,shared_value,share_net,share_optimizer,device,lock,i)
