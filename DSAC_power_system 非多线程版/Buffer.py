...
class Replay_buffer():
    ...
    def sample(self, batch_size, epsilon = 1e-6):
        ...
        for i in ind:
            ...
            state.append(np.array(S.reshape(4,), copy=False))
    ...
    def run(self):
        print("fuck_buffer0",self.experience_in_queue.empty(),self.args.initial_buffer_size)
        while not self.stop_sign.value:
            if not self.experience_in_queue.empty():
                self.push(self.experience_in_queue.get())
                print("fuck_buffer1",len(self.storage))
            else:
        ...
        print("buffer12",self.experience_in_queue.empty())
def test():
    def fff(x):
        return x,x+1,x+2
    print(fff(1)[0:2])
